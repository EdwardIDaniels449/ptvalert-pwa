...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('[纯Google Maps修复] DOM加载完成');
        initMapFix();
    });
    
    // 如果DOM已加载，立即执行
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        console.log('[纯Google Maps修复] DOM已加载，立即初始化');
        initMapFix();
    }
    ...
    // 地图就绪事件处理函数
    function onMapReady() {
        console.log('[纯Google Maps修复] 地图就绪事件触发');
        mapReady = true;
    ...
    function safelyPatchAddMarkerFunction() {
        if (!window.UIController) {
            console.warn('[纯Google Maps修复] UIController不存在，等待创建');
            setTimeout(safelyPatchAddMarkerFunction, 1000);
            return;
        }
        
        console.log('[纯Google Maps修复] 修补添加标记函数');
        
        // 保存原始函数，如果还没保存的话
        if (!originalAddReportMarker && window.UIController.addReportMarker) {
            originalAddReportMarker = window.UIController.addReportMarker;
        }
        ...
        window.UIController.addReportMarker = function(location, description, reportId, image) {
            // 确保地图就绪
            if (!isMapReady()) {
                console.warn('[纯Google Maps修复] 地图未就绪，将标记加入待处理队列');
            ...
            // 调用原始函数
            try {
                if (originalAddReportMarker) {
                    return originalAddReportMarker(location, description, reportId, image);
                } else {
                    console.warn('[纯Google Maps修复] 未找到原始的addReportMarker函数');
                    return createBasicMarker(location, description);
                }
            } catch (e) {
                console.error('[纯Google Maps修复] 添加标记时出错:', e);
                return createBasicMarker(location, description);
            }
        };
    }
    ...
    function createBasicMarker(location, description) {
        if (!window.google || !window.google.maps || !window.map) {
            console.warn('[纯Google Maps修复] 无法创建基本标记：API或地图不可用');
            return null;
        }
        ...
        try {
            ...
            console.error('[纯Google Maps修复] 创建基本标记失败:', e);
            return null;
        }
    }
    ...
    function ensureGoogleMapsLoaded() {
        ...
        if (typeof google !== 'undefined' && google.maps && google.maps.Map) {
            console.log('[纯Google Maps修复] Google Maps API已加载');
            checkMapObject();
            return;
        }
        
        console.log('[纯Google Maps修复] 等待Google Maps API加载');
        ...
        if (typeof window.googleMapsLoadedCallback === 'function') {
            ...
            window.googleMapsLoadedCallback = function() {
                ...
                // 我们的附加处理
                console.log('[纯Google Maps修复] Google Maps API加载回调触发');
                setTimeout(checkMapObject, 1000);
            };
        }
        
        // 检查API脚本是否存在
        const hasMapScript = !!document.querySelector('script[src*="maps.googleapis.com"]');
        
        if (!hasMapScript) {
            console.warn('[纯Google Maps修复] 没有发现Google Maps脚本，尝试加载');
    ...
    function loadGoogleMapsScript() {
        ...
        window.pureMapFixCallback = function() {
            console.log('[纯Google Maps修复] 手动加载的Google Maps API就绪');
            setTimeout(checkMapObject, 1000);
        };
        
        // 添加错误处理
        script.onerror = function() {
            console.error('[纯Google Maps修复] Google Maps脚本加载失败');
        };
    ...
    function checkMapObject() {
        // 检查全局地图对象
        if (!window.map) {
            console.warn('[纯Google Maps修复] 全局地图对象不存在');
            
            // 检查地图容器
            const mapElement = document.getElementById('map');
            if (!mapElement) {
                console.error('[纯Google Maps修复] 地图容器不存在');
                return;
            }
            
            // 如果API已加载但地图不存在，尝试创建地图
            if (typeof google !== 'undefined' && google.maps && google.maps.Map) {
                console.log('[纯Google Maps修复] 尝试创建新的地图对象');
                ...
                try {
                    ...
                    console.error('[纯Google Maps修复] 创建地图对象失败:', e);
                }
            }
        } else {
            console.log('[纯Google Maps修复] 全局地图对象已存在');
            ...
            if (isValidMapObject(window.map)) {
                ...
                console.warn('[纯Google Maps修复] 地图对象无效');
            }
        }
    }
    ...
    function processPendingMarkers() {
        ...
        if (!isMapReady()) {
            console.warn('[纯Google Maps修复] 地图未就绪，无法处理待处理标记');
            return;
        }
        
        console.log(`[纯Google Maps修复] 处理 ${window.pendingMarkers.length} 个待处理标记`);
        
        // 创建副本并清空原数组
        const markers = window.pendingMarkers.slice();
        ...
        markers.forEach(function(marker) {
            try {
                ...
                console.error('[纯Google Maps修复] 处理待处理标记时出错:', e);
                createBasicMarker(marker.location, marker.description);
            }
        });
    }
    ...
    function isValidMapObject(mapObj) {
        ...
        // 检查是否有必要的方法
        const hasMethods = typeof mapObj.getCenter === 'function' && 

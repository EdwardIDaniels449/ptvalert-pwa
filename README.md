...
一个用于实时报告和查看事件的地图应用，采用渐进式Web应用(PWA)技术，可在所有平台使用。

## 功能特点

- **离线支持**：即使在没有网络连接的情况下也能使用核心功能
- **添加到主屏幕**：可像原生应用一样安装到设备主屏幕
- **实时更新**：查看和添加实时事件报告
- **地图集成**：直观地在地图上显示事件
- **用户认证**：安全的登录和用户管理
- **推送通知**：获取重要事件的通知(仅Android和桌面浏览器支持)
...
1. **准备应用图标**：
   - 在`/images/`目录下需要以下尺寸的图标：
     - 72×72, 96×96, 128×128, 144×144, 152×152, 192×192, 384×384, 512×512 像素
   - 还需要特殊格式的图标：
     - icon-512x512-maskable.png (带安全区域的适配图标)
     - badge-96x96.png (通知徽章图标)
     - report-icon-192x192.png 和 map-icon-192x192.png (快捷方式图标)

2. **准备iOS启动屏**：
   - 需要为不同设备准备启动屏图像，放在`/images/`目录下
   - 主要尺寸包括：2048×2732, 1668×2388, 1668×2224, 1536×2048, 1242×2688, 1125×2436, 828×1792, 750×1334, 640×1136 像素
...
1. **配置HTTPS**：
   - PWA必须通过HTTPS提供服务才能使用Service Worker
   - 建议使用Netlify, Vercel, Firebase Hosting等服务，它们自动提供HTTPS
   - 或在自己的服务器上使用Let's Encrypt获取免费SSL证书

2. **设置正确的MIME类型**：
   - 确保服务器为`manifest.json`提供`application/json` MIME类型
   - 确保Service Worker文件提供`application/javascript` MIME类型

3. **上传文件**：
   - 将所有文件上传到你的网络服务器或托管服务
   - 确保Service Worker文件位于网站根目录，以便其能控制整个应用范围

4. **测试PWA功能**：
   - 使用Chrome DevTools的Lighthouse或PWA工具测试PWA功能
   - 测试应用在不同设备上的安装体验
   - 验证离线功能是否正常工作
...
- iOS上不支持Web推送通知
- 每次从主屏幕打开后，PWA会重启(不保持状态)
- 需要通过Safari的"添加到主屏幕"选项安装

### Android特有考虑

- 支持完整的PWA功能，包括推送通知
- 支持应用内更新提示
- 通过Chrome的安装选项或添加到主屏幕来安装

### 桌面特有考虑

- Chrome, Edge, Opera等现代浏览器支持完整PWA安装
- Firefox支持部分PWA功能
- Safari(macOS)支持有限的PWA功能，不支持安装到桌面

## 故障排除

- **无法安装应用**：确保网站通过HTTPS提供服务
- **离线功能不工作**：检查Service Worker是否正确注册
- **iOS上显示空白**：检查iOS专用元标签是否正确设置
- **无法接收推送通知**：确认设备和浏览器支持Web推送API
